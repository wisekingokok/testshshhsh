...
public class GetWordTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    private String[] highlightTextArray = new String[]{"The","of","by"};
...
//            return;
//        }else {
//            for (int i = 0;i<highlightTextArray.length;i++){
//                int hIndex = mText.toString().indexOf(highlightTextArray[i]);
//                while (hIndex != -1) {
//                    spannableString.setSpan(new ForegroundColorSpan(highlightColor), hIndex, hIndex + highlightTextArray[i].length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
//                    hIndex += highlightTextArray[i].length();
